...
const Card: React.FC<CardProps> = ({film}) => {
  ...
  return (
      <View onTouchEnd={setOpenedFilm}>
        <ImageBackground style={CardStyles.wrapper} source={{uri: film.backdrop_path ? `https://image.tmdb.org/t/p/original${film?.backdrop_path}` : 'https://i.ytimg.com/vi/317jz-PU7Mg/maxresdefault.jpg'}}>
        <View style={CardStyles.footer}>
            <Text style={CardStyles.title}>{film.title}</Text>
            <Text style={CardStyles.year}>{film.release_date?.split('-')[0]}</Text>
        </View>
        </ImageBackground>
    </View>
  );
